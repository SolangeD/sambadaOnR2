...
{	
	...
	for (int i(T); i>=A; --i)	// On ne prend pas la valeur manquante "M" en compte pour l'étiquettage
	{
		...
		/*		for (int j(0); j<etiquette.size(); ++j)
		 {
		 cout << etiquette[j] << " ";
		 }
		 cout << endl; */
	...
	//string nomFichierPlink, nomFichierSortie
	string nomFichierSel, nomFichierPed, nomFichierMap;
	...
	/*if (argc < 5)
	{
		...
		cerr << "Usage : Recodage nbSamples nbSNP PlinkFile outputFile (subsetSamplesFile)" << endl;
	...
	for (int i(0); i<nbSel; ++i)
	{
		...
		for (int j(0); j<nbSNP; ++j)
		{
			if (caracSNP[j].first)
			{
				sortie << caracSNP[j].second->second.valeurs[data[i][j]-premSymbole];
			}
		}
