...
{}
//const string&
int Supervision::preparationsCalculs(std::string nomFichier)
{
	...
	if (entreeFichier.fail())
	{
		...
		return(2); //Au lieu de exit(2) pour que R ne se vexe pas
	}
...
int Supervision::fusionResultats(std::string nomFichierInput, int nbBlocs, int tailleBlocs, int dimensionMax, typeScore scoreSel=Both, double seuilScore=0, typeScore scoreTri=Wald, char delimMots=' ') throw()
{
	time_t temps_start(time(NULL));
	...
	nomFichierMarq.first=nomFichierInput; //change
