...
#' @return None (to be changed)
#' @examples
#' prepare_geno('myPlinkFile.ped',maf.tresh=0.05, missingness.thresh=0.05,LD.thresh=0.2,mgf.thresh=0.8,interactiveChecks=TRUE)
#' prepare_geno('myGDSFile.gds',maf.tresh=0.05, missingness.thresh=0.05,LD.thresh=0.2,mgf.thresh=0.8,interactiveChecks=FALSE)
prepare_geno=function(filename,maf.thresh=NULL, missingness.thresh=NULL,LD.thresh=NULL,mgf.thresh=NULL,interactiveChecks=FALSE, verbose=FALSE){

  ### Check inputs ###
  
  if(typeof(filename)!='character') stop("filename argument supposed to be decimal number")
  ...
  if(interactiveChecks==TRUE){ #If true, shows histogram of pruning values and asks the user to check chosen thresholds
    snpSummary=SNPRelate::snpgdsSNPRateFreq(gds_obj)
    ...
    if(is.null(LD.thresh)==FALSE){
      ...
      for(ld in seq(0,1,0.05)){
        snp_pruned[i]=length(unlist(snpgdsLDpruning(gds_obj, ld.threshold=ld, verbose=FALSE, remove.monosnp=FALSE)))
        i=i+1
      }
  ...
  if(!is.null(mgf.thresh)){
    ...
    list_snp=snpMGF[snpMGF %in% unlist(gds_pruned)] 
  }
  else{
    list_snp=unlist(gds_pruned)
  }
  ...
  recodePlink(numIndiv,numSNP,paste(filename_short,'_filtered',sep=''),paste(filename_short,'.csv'))
}
...
#' @param worldclimbio logical If TRUE worldclim bio variables will be downloaded at a resolution of 0.5 minutes of degree (the finest resolution). Rely rgdal and gdalUtils R package to merge the tiles. The downloaded tiles will be stored in the (new) wc0.5 directory of the active directory
...
#' @return None 
#' @examples
#' create_env(rastername=c('prec.tif','tmin.sdat'),locationfilename='MyFile.shp',rasterproj=c(4326,21781), worldclimbio=TRUE,interactiveChecks=TRUE)
#' create_env(locationfilename='MyFile.csv',x='Longitude',y='Latitude',locationproj=4326, worldclimbio=TRUE,interactiveChecks=FALSE)
create_env=function(rastername, rasterproj,directory=FALSE, locationfilename, x,y,locationproj, separator=',', worldclimbio=TRUE, srtm=FALSE, interactiveChecks, verbose=TRUE){
  ...
  if(worldclimbio==TRUE | srtm==TRUE){
  ...
  if(!is.null(worldclimbio)){
    if(typeof(worldclimbio)!='logical') stop("worldclimbio is supposed to be logical")
  }
  if(!is.null(srtm)){
  ...
  if(is.null(rastername) & worldclimbio==FALSE & srtm==FALSE) stop('Either provide a rastername or set worldclimbio or srtm to true')
  ...
  if(is.na(raster::projection(locations)) & !is.null(locationproj)){
    raster::projection(locations) = CRS(paste0('+init=epsg:',locationproj))
  }
  ...
  if(!is.null(rastername)){
    ...
    for(i in 1:length(rastername)){
      ...
      if(is.na(raster::projection(raster)) & !is.null(rasterproj)){
        ...
        raster::projection(raster) = CRS(paste0('+init=epsg:',rasterproj2))
      }
  ...
  if(worldclimbio==TRUE){
    
    if(verbose==TRUE){
      print('Downloading bioclim variables from Worldclim database')
    }
    
    setwd(active_dir)
    
    #Load wordclim
    locations2=sp::spTransform(locations,CRS('+init=epsg:4326'))
    coord=sp::coordinates(locations2)
    
    for(i in 1:nrow(locations)){
      latt=coord[i,"Latitude"]
    ...
    for(i in 1:19){ #Loop on number of bio
      ...
      gdalUtils::mosaic_rasters(files, paste0('bio',i,'.tif'), verbose=FALSE)
    ...
    raster::projection(raster)=CRS("+init=epsg:4326")
    if(interactiveChecks==TRUE){
      plot(raster, y=1,main='bio1')
  ...
  if(srtm==TRUE){
    ...
    for(i in 1:nrow(locations)){
      ...
      if(!file.exists(paste0('srtm_',colTile,'_',rowTile,'.tif'))){ #Check if file already downloaded. Should be done by getData but bugs
        ...
        tryCatch({raster=getData("SRTM",path=active_dir, lon=long, lat=latt)}, error=function(e){})        
      }

    }
    
    # Merge all files
    files=paste0('srtm_',colTiles,'_',rowTiles,'.tif')
    gdalUtils::mosaic_rasters(files,'srtm.tif', verbose=FALSE)
    
    #Load raster
    raster=raster('srtm.tif')
  ...
  write.table(data, file=paste0(locationfilename_short,'-env.csv'), append=FALSE,quote=FALSE,sep=" ", dec = ".",row.names=FALSE,col.names=TRUE)
...
#' @param numpc double If above 1, number of principal components to analyze. If between 0 and 1, automatic detection of number of PC 
...
#'# @param LD.thresh double A number between 0 and 1 specifying the linkage desiquilibrium (LD) rate filtering (if null no filtering on missing rate will be computed)
#' @param numpop integer If not null, clustering based on \ref{numpop} first PC will be computed to devide into \ref{numpop} populations. If -1 automatic detection of number of cluster (elbow method if \ref{ClustMethod}='kmeans', maximise branch length if \ref{ClustMethod}='hclust')
...
#' prepare_geno('myPlinkFile.ped',maf.tresh=0.05, missingness.thresh=0.05,LD.thresh=0.2,mgf.thresh=0.8,interactiveChecks=TRUE)
#' prepare_geno('myGDSFile.gds',maf.tresh=0.05, missingness.thresh=0.05,LD.thresh=0.2,mgf.thresh=0.8,interactiveChecks=FALSE)
prepare_env=function(envfile, maxcorr, idname, separator=',',genofile, numpc, maf.thresh, missingness.thresh, numpop, ClustMethod='kmeans', includeCol=NULL, excludeCol=NULL, popstrCol=NULL, x,y,locationproj,interactiveChecks=FALSE, verbose=TRUE, pca){
  ...
  if(!is.null(numpop)){ 
    if(numpop<0) stop("Numpop must be positive")
  ...
  for(i in 1:ncol(env2)){
    #If column is numeric and not ID nor in excludeCol
  ...
  ### Calculate propulation struction
  
  if(!is.null(genofile)){ #If genofile is not null, calculate population structure
    ...
    #Retrieve right number of population
    popvect2=popvect2[1:numpop] #First column=ID, keep numpop-1 populations
    colnames(popvect2)=c('sampleid',paste0('pop',1:(numpop-1)))
    ...
    # Plot VP of first 100 axes and show chosen number of population
    if(interactiveChecks==TRUE){
      ...
      numpc2 = readline(prompt="Would you like to choose a different number of population? (press any letter if no, or enter a new number): ")
    ...
    if(!is.null(numpop)){
      ...
      while(changePopNum==TRUE){ #The user can interactively change the number of population. Loop begins again 
        ...
        if(interactiveChecks==TRUE & !is.null(x)){
            ...
            plot(get(country),xlim=c(min(sp::coordinates(location)[,'Longitude']),max(sp::coordinates(location)[,'Longitude'])),ylim=c(min(sp::coordinates(location)[,'Latitude']),max(sp::coordinates(location)[,'Latitude'])))
            rad=abs(max(sp::coordinates(location)[,'Longitude'])-min(sp::coordinates(location)[,'Longitude']))/50
            for(i in 1:nrow(location)){
              mapplots::add.pie(z=1/dist[i,1:(numpop)],x=coordinates(location[i,1])[,'Longitude'], y=coordinates(location[i,1])[,'Latitude'], labels=NA, radius=rad)
            }
            numpop2 = readline(prompt="Would you like to choose a different number of population? (press any letter if no, -1 to reshow the tree/elbow, or enter a new number): ")
            ClustMethod2 = readline(prompt="Would you like to choose a different clustering alogrithm? (press any letter if no, kmeans or hclust to change algorithm): ")
            if(grepl("[[:digit:]\\.-]",numpop2) | ClustMethod2=='hclust' | ClustMethod2=='kmeans'){
  ...
  if(!is.null(genofile) | !is.null(popstrCol)){
    ...
    if(!is.null(popstrCol)){
      ...
      numvar=(length(popstrCol)+1)
      popvect2=env[,popstrCol]
    } else if(!is.null(numpop)){
      popvect2=dist[,1:(numpop-1)]
    } else {
      numvar=numpc
    }
    for(i in 1:(numvar-1)){
      for(j in 1:ncol(env_kept)){
        ...
        corr_value=cor(env_kept[,j],as.numeric(popvect2[,i]))*100
        if(corr_value>70){
          # Inform user if population and env var are more than 70% correlated
          print(paste0('!! Variable ',colnames(env_kept)[j],' is correlated at ',corrvalue,' percent with population ',colnames(popvect2)[i],' !!'))
        }
  ...
  if(!is.null(x)){
    if(!is.null(genofile)){
      ...
      colnames(total)=c(idname,x,y,colnames(kept_env),colnames(popvect2)[2:numvar])
    } else {
      total = cbind(env[,idname],env_kept)
      colnames(total)=c(idname,colnames(kept_env))
    }
  } else { #not geographic column
    if(!is.null(genofile)){
      #ID match between envfile and genofile
      total=cbind(env[,idname],env_kept,popvect2)
      colnames(total)=c(idname,colnames(kept_env),colnames(popvect2)[2:numvar])
    } else {
      total = cbind(env[,idname],env_kept)
      colnames(total)=c(idname,colnames(kept_env))
    }    
...
extractVar = function(raster, locations){
  if(raster::projection(locations)!=raster::projection(raster)){ #marche pas forcement car peut avoir init=epsg en plus
    locations2=sp::spTransform(locations, CRS(raster::projection(raster)))
  } 
...
createGDSfile=function(filename){
  ...
  if (extension=='gds') {
    ...
    SNPRelate::snpgdsBED2GDS(bed.fn=filename,fam.fn=paste(filename_short,'.fam',sep=''), bed.fn=paste(filename_short,'.fam',sep=''),gds_file, verbose=FALSE)
